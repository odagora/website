...
if (isset($_POST['careers-submit'])) {
    if ($_POST['career-name'] != '' && $_POST['career-email'] != '' && $_POST['birth-date'] != '' && $_POST['experience-years'] != '' && $_POST['career-position'] != '') {
        $message_content = '';
        $detect_errors = $_SERVER['HTTP_REFERER'] ? '<br><br><br><br>' . __('This Message Was Sent From: ', 'kyma') . $_SERVER['HTTP_REFERER'] : '';

        $receiver_mail = get_option('admin_email'); // Add a recipient
        ...
        $mail->Subject = __("Employment application for: ", "kyma") . $client_careers_position;

        $client_name = isset($client_name) ? __("Name: ", "kyma") . $client_name : '';
        $client_email = isset($client_email) ? "<br><br>" . __("Email: ", "kyma") . $client_email : '';
        $client_address = isset($client_address) ? "<br><br>" . __("Address: ", "kyma") . $client_address : '';
        $client_birth_date = isset($client_birth_date) ? "<br><br>" . __("Date of birth: ", "kyma") . $client_birth_date : '';
        $client_experience_years = isset($client_experience_years) ? "<br><br>" . __("Years of experience: ", "kyma") . $client_experience_years : '';
        $client_careers_position = isset($client_careers_position) ? "<br><br>" . __("Job title: ", "kyma") . $client_careers_position : '';
        $client_message = isset($client_message) ? "<br><br>" . __("Message: ", "kyma") . $client_message : '';
		...
		if(isset($_FILES["cv-attachment"]["name"])){
        ...
        if ($succesfully_send == true) {
            echo __('Your application has been <strong>successfully</strong> sent, We will send you a reply as soon as possible.', 'kyma');
			die;
        } else {
            echo __('Email <strong>could not</strong> be sent due to some Unexpected Error. Please Try Again.<br /><br /><strong>Reason:</strong><br />', 'kyma') . $not_send_reasons . '';
			die;
        }
