...
function kyma_shortcode_team($atts, $content = null)
{
    ...
    switch ($style) {
                    ...
                    <div class="col-md-3">
                        <div class="team_block flipp_effect">
                            <div class="f1_card">
								...
								<span class="team_img">
									<?php if (has_post_thumbnail()) the_post_thumbnail('team_circle_thumb'); ?>
								</span>
        ...
        case 2:
            ?>
            <div class="team_block3 rows_container clearfix"><?php $all_posts = wp_count_posts('kyma_team')->publish;
            $args = array('post_type' => 'kyma_team', 'posts_per_page' => $all_posts);
            $team = new WP_Query($args);
            if ($team->have_posts()) {
                while ($team->have_posts()): $team->the_post();
                    ...
                    <div class="team-col no_padding clearfix" data-color="#0BACB8">
                        <div class="team-col-1">
                            <a class="member_img2" href="<?php echo esc_url($url[0]); ?>" data-rel="magnific-popup">
							<span>
								<?php if (has_post_thumbnail()) the_post_thumbnail('team_square_thumb'); ?>
							</span>
                            </a>
                        </div>
...
function kyma_portfolio_shortcode($atts, $content = null){
    ...
    <!-- Filter Content -->
    <div
        class="hm_filter_wrapper <?php echo esc_attr($port_col); ?>_blocks <?php echo esc_attr($width); ?>_portos has_sapce_portos project_text_nav nav_with_nums upper_title upper_title"><?php
        $Terms = array_search('all', explode(', ', $port_cat)) ? '' : explode(', ', $port_cat) ;
        $terms = get_terms('portfolio_taxonomy', array('orderby'=>'count', 'order'=>'desc', 'hide_empty'=>true,'slug'=>$Terms));
        if (!empty($terms) && $hide_filter=='0' && !is_wp_error($terms)) {
            ...
            <ul data-option-key="filter" class="option-set clearfix" id="filter-by">
                <?php 
                if ($Terms!="") {?>
                    <li><a data-option-value="*" class=""
                           href="#"><span><?php _e('All','kyma'); ?></span><span
                                class="num"></span></a>
                    </li>
                <?php }
        ...
        <div class="hm_filter_wrapper_con"><?php
            ...
            if ($kyma_portfolio->have_posts()) {
                while ($kyma_portfolio->have_posts()) : $kyma_portfolio->the_post();
                    $term_list = wp_get_post_terms(get_the_ID(), 'portfolio_taxonomy', array("fields" => "names"));
                    $terms = implode(" ", $term_list);?>
                    <div class="filter_item_block <?php echo esc_attr($terms); ?>">
                        <div class="porto_block">
                            <div
                                class="porto_type"><?php if (metadata_exists('post', get_the_ID(), '_kyma_portfolio_gallery')) {
        ...
        <div class="content row_spacer clearfix"><?php
            if ($testimonial_heading != "") {
                ...
                <h2 id="testimonial_heading"><span
                        class="line"></span><?php echo esc_attr($testimonial_heading); ?></h2>
                </div><?php
            } ?>
        ...
        <div class="content row_spacer clearfix"><?php
            if ($testimonial_heading!= "") {
                ...
                <h2><span class="line"><i
                            class="ico-comments-o"></i></span><?php echo esc_attr($testimonial_heading); ?>
                </h2>
                </div><?php
            } ?>
            <!-- Content Slider -->
