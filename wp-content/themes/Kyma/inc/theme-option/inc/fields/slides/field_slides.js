            ...
            function () {
                ...
                el.find('.redux-slides-remove').live(
                    'click', function () {
                        ...
                        $(this).parent().siblings().find('input[type="date"]').val('');
                ...
                el.find('.redux-slides-add').off('click').click(
                    function () {
                        ...
                        var slideType = $(this).attr('rel-id');
                        if (slideType == 'webhunt_service-ul') {
                            max = 999;
                        }
                        if (slideCount1 < max) {
                            $(newSlide).find('input[type="text"],input[type="date"],select ,input[type="hidden"], textarea').each(
                                function () {
                            ...
                            var section = $(this).prev().find('.redux-slides-accordion-group fieldset:first-child').attr('data-id');
                            var stack = section.split('_');
                            alert('In FREE version you can add only ' + max + ' ' + stack[1]);
                        }
                    }
                );
