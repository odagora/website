 ...
 * Description: ChatBot is a WordPress Chat Bot plugin to provide quick support and email functionality.
...
define('QCLD_wpCHATBOT_VERSION', '3.3.5');
...
class qcld_wb_Chatbot
{
    ...
    public function qcld_wb_chatbot_admin_menu()
    {
       /* add_submenu_page('wpcommerce',
        ...
        add_menu_page( 'WPBot Lite', 'WPBot Lite', 'manage_options','wpbot-panel', array($this, 'qcld_wb_chatbot_admin_page'),'dashicons-format-status', 6 );

		add_submenu_page( 'wpbot-panel', 'Settings', 'Settings', 'manage_options','wpbot', array($this, 'qcld_wb_chatbot_admin_page_settings') );
		
		$hook = add_submenu_page( 'wpbot-panel', 'Simple Text Responses', 'Simple Text Responses', 'manage_options','simple-text-response', array($this, 'qcld_wb_chatbot_admin_str') );

        add_action( "load-$hook", [ $this, 'screen_option' ] );
		
		add_submenu_page( 'wpbot-panel', 'Support', 'Support', 'manage_options','wpbot_support_page', 'qcpromo_support_page_callback_func' );
		
		add_submenu_page( 'wpbot-panel', 'Help', 'Help', 'manage_options','wpbot_help_page', 'wpbot_help_page_callback_func' );
    }
    ...
    public function qcld_wb_chatbot_admin_scripts($hook)
    {
        ...
        if (((!empty($_GET["page"])) && ($_GET["page"] == "wpbot")) || ($hook == "widgets.php") || $_GET['page']=='wpbot_help_page' || $_GET['page']=='wpbot-panel') {
            ...
            wp_register_script('qcld-wp-chatbot-admin-js', plugins_url(basename(plugin_dir_path(__FILE__)) . '/js/qcld-wp-chatbot-admin.js', basename(__FILE__)), array('jquery', 'jquery-ui-core','jquery-ui-sortable','wp-color-picker','qcld-wp-chatbot-timepicker-js'), true);
    ...
    public function qcld_wb_chatbot_frontend_scripts()
    {
        ...
        $wp_chatbot_obj = array(
            ...
            'whats_off_page' => $this->qcld_wb_chatbot_preg_replace_url(get_option('qlcd_wp_chatbot_whats_off_page')),
        ...
        //Check for current time to change Whatsapp CTA on start menu
        $whatsapp_status = array();
        if(!empty(unserialize(get_option('qlcd_wp_chatbot_whats_label'))[0])){
            if(strpos(get_option('qc_wpbot_menu_order'), unserialize(get_option('qlcd_wp_chatbot_whats_label'))[0])){
                if(get_option('enable_wp_chatbot_opening_hour') == 1){
                    if(wp_chatbot_check_opening_hours() === false){
                        $whatsapp_status = array('disabled');
                    }
                    else{
                        $whatsapp_status = array('enabled');
                    }
                }
            }
            wp_localize_script('qcld-wp-chatbot-plugin', 'whatsapp_status', $whatsapp_status);
        }
    }
    ...
    //getting sanitized url path on text input
    public function qcld_wb_chatbot_preg_replace_url($text){
        $refined_text = preg_replace('/\s+/', '-', sanitize_text_field(strtolower($text)));
        return $refined_text;
    }
    ...
    {
        ?>
        <h4 class="qc-opt-title"><?php _e($option_text, 'wpchatbot'); ?></h4>
        <div class="wp-chatbot-lng-items">
            ...
            if (is_array($options) && count($options) > 0) {
                ...
                <div class="row" class="wp-chatbot-lng-item">
                    ...
                    <div class="col-xs-2">
                        <span class="wp-chatbot-lng-item-remove"><?php _e('X', 'wpchatbot'); ?></span>
                    </div>
                </div>
            <?php } ?>
        </div>
    ...
    public function wp_chatbot_opening_hours($day_name,$wpwbot_times){
        ...
        ?>
            <div class="wp-chatbot-hours-container">
                <div class="wp-chatbot-hours">
                    <input type="text" class="wp-chatbot-hour" name="wpwbot_hours[<?php echo $day_name; ?>][<?php echo $segment; ?>][]" value="<?php if(isset($day_time[0])){echo $day_time[0];}else{ echo "00:00";}  ?>" >
                    <input type="text" class="wp-chatbot-hour" name="wpwbot_hours[<?php echo $day_name; ?>][<?php echo $segment; ?>][]" value="<?php if(isset($day_time[1])){echo $day_time[1];}else{ echo "00:00";}  ?>" >
                </div>
        ...
        ?>
            <div class="wp-chatbot-hours-container">
                <div class="wp-chatbot-hours">
                    <input type="text" class="wp-chatbot-hour" name="wpwbot_hours[<?php echo $day_name; ?>][0][]" value="00:00" > <input type="text" name="wpwbot_hours[<?php echo $day_name; ?>][0][]" value="00:00">
                </div>
                <div class="wp-chatbot-hours-remove">
                    <button type="button" class="btn btn-danger btn-sm wp-chatbot-hours-remove-btn">
                        <i class="fa fa-times" aria-hidden="true"></i>
        ...
        if (isset($_POST['_wpnonce']) && $_POST['_wpnonce']) {
            ...
            if (isset($_POST['submit'])) {
                ...
                if (isset($_POST["wp_chatbot_position_x"])) {
                    $wp_chatbot_position_x = stripslashes(($_POST["wp_chatbot_position_x"]));
                    update_option('wp_chatbot_position_x', $wp_chatbot_position_x);
                }
                if (isset($_POST["wp_chatbot_position_y"])) {
                    $wp_chatbot_position_y = stripslashes(($_POST["wp_chatbot_position_y"]));
               ...
               if(isset( $_POST["disable_wp_chatbot"])){
                   $disable_wp_chatbot = $_POST["disable_wp_chatbot"];
                ...
                update_option('qlcd_wp_chatbot_admin_email', stripslashes($qlcd_wp_chatbot_admin_email));
                ...
                if(isset( $_POST["enable_wp_chatbot_rtl"])) {
                    $enable_wp_chatbot_rtl = $_POST["enable_wp_chatbot_rtl"];
				...
				if(isset( $_POST["show_menu_after_greetings"])) {
                    $show_menu_after_greetings = $_POST["show_menu_after_greetings"];
               ...
               if(isset( $_POST["enable_wp_chatbot_mobile_full_screen"])) {
                ...
                // upload custom wpwbot icon path
                 ...
                 //Agent image
                //wpwbot icon settings.
                $wp_chatbot_icon = (isset($_POST['wp_chatbot_agent_image']) ? $_POST['wp_chatbot_agent_image'] : 'agent-0.png');
                ...
                //Theming
                $qcld_wb_chatbot_theme = (isset($_POST['qcld_wb_chatbot_theme']) ? $_POST['qcld_wb_chatbot_theme'] : 'template-00');
                ...
                if(isset( $_POST["qcld_wb_chatbot_change_bg"])) {
                    $qcld_wb_chatbot_change_bg = $_POST["qcld_wb_chatbot_change_bg"];
				...
				if(isset($_POST["qcld_wb_chatbot_board_bg_path"])){
					$qcld_wb_chatbot_board_bg_path = $_POST["qcld_wb_chatbot_board_bg_path"];
				...
				if(isset($_POST["wp_chatbot_custom_css"])){
					$wp_chatbot_custom_css = $_POST["wp_chatbot_custom_css"];
               ...
               //help welcome and message
                ...
                $qlcd_wp_chatbot_whats_off_page = @$_POST["qlcd_wp_chatbot_whats_off_page"];
                update_option('qlcd_wp_chatbot_whats_off_page', sanitize_text_field($qlcd_wp_chatbot_whats_off_page));
               //Viber
...
function qcld_wb_chatboot_defualt_options(){
  ...
  if(!get_option('qlcd_wp_chatbot_messenger_label')) {
    ...
    if(!get_option('qlcd_wp_chatbot_whats_off_page')) {
            update_option('qlcd_wp_chatbot_whats_off_page', $path);
        }
    //Viber
...
function qcld_wb_chatboot_delete_all_options(){
    ...
    delete_option( 'qlcd_wp_chatbot_whats_off_page');
    // Viber
...
function wpbot_help_page_callback_func(){
	...
	<div class="container" style="margin-right:unset;margin-left:unset;">
		<h1>Help</h1>
		<div class="content form-container" style="padding: 10px;color:#fff">
		
			<h2 style="font-size:20px">Tutorials</h2>
			...
			<h3 style="font-size:20px">Check out the Pro version of the Plugin for more features from <a href="https://www.quantumcloud.com/products/chatbot-for-wordpress/" target="_blank">here</a>.</h3>
		</div>
	</div>
